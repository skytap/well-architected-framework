...
Within {{site.Brand}}, there are five areas to build a control framework around: platform, resource management, internal networking, edge networking, and virtual machines.
...
Users, Groups, Projects, and Departments are used to control cost and access to services. They should be configured to align with your organization's best business processes and operational best practices.
...
{{site.Brand}} has additional permissions that can be enabled or disabled for each user. Some of these permissions are displayed only when specific features are enabled in the platform. The table below shows the additional permissions that are **optional (O)** for each type of user.
...
Groups help you manage large numbers of user permissions across multiple projects. A user can belong to multiple groups and the groups can be given permissions within {{site.Brand}}. The platform supports up to 100 groups; groups are managed by administrators and user managers.

### Projects

Projects grant users access to other resources within {{site.Brand}}. Users always use the most permissive access rights granted, so if a named user is granted Editor rights but is also member of a group with Viewer permissions, they can use the Editor rights to perform actions on the project.

Projects support the concept of an Automatic Role; this role is granted to every new user on {{site.Brand}}. This feature should be used sparingly to prevent excessive user permissions.
...
| √             | √       | √      |             |        | Share resources with other projects (as Editor or Manager in both projects)                             |
...
‡  _The restricted user must be an editor or manager in at least one project to perform these actions as a participant. For example, if the user is a participant in project A and an editor in project B, the user can create an environment from a template in project A. The new environment is automatically added to project B (where the user has permission to add resources). Restricted users cannot own environments, templates, or assets outside of a project._
...
* **Golden templates** – Use this project to share golden templates with all users.
  ...
  * Add all other {{site.Brand}} users to the project in the **Participant** role. **Participants** can create environments from templates in the project.

Enable an **automatic role** on the project. With this setting, every new {{site.Brand}} user is automatically added to this project as a **Participant**.
...
* **Staging** – Use this project to share in-progress environments and templates between template creators.
  ...
  * Do not add other {{site.Brand}} users to this project. Because they don't have project access, they can't view or use environments and templates in this project.

#### Departments

Departments allow you to model company departments, business units, or project teams within the {{site.Brand}}. With department monitoring and limits, you can:

* Create usage limits to cap the amount of storage, RAM hours, or concurrent RAM that department users can consume; this can guarantee that {{site.Brand}} resources remain available to critical departments.
...
Auditing can be configured to send events to a log management tool or Security Incident Event Monitoring (SIEM) tool. These events are sent using webhooks, allowing for near real-time notifications to management tooling enabling security engineers to respond more rapidly than would otherwise be available.
