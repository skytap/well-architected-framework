---
title: Data Protection and Recovery for IBM i Series
description: Commvault Cold Backup and Migration Partner Solutions
author: Commvault
---

# Data Protection and Recovery for IBM i Series

Commvault's IBM i iDataAgent provides the data protection, recovery and management of supported data types and levels of granularity needed to for IBM i OS and IFS architectures.
...
* [IBM i iDataAgent key use cases](#dataagentusecases)
  * [Backup and recovery of the IBM i Integrated File System (IFS)](#ifsbackuprecovery)
  ...
  * [Enabling Disaster Recovery and IBM i systems Life Cycle Management](#DRandLCM)
* [Supported IBM i systems](#supportedsystems)
...
Included as part of Commvault's overall data management platform the scalability, operational efficiency, and functionality of the IBM i Series backup and recovery agent far exceeds the abilities of other IBM i data protection options, including the BRMS utility. With the release of the Version 11, Service Pack 4 of the software, the iDataAgent is more streamlined and has a number of options designed to improve backup performance and recovery ease.
...
<a name="complianceandriskmanagement"></a>
...
_With the End of Support (EOS) of TSM 6.2, access to TSM Servers through the TSM API on IBM i will also reached EOS. TSM 6.2 was the* **last** version of TSM to support IBM i. No further versions of TSM will have API support for IBM i._

_IBM TivoliStorage Manager (TSM) Version 6.2 reached End of Support (EOS) on April 30, 2015._

<a name="commcellhierarchy"></a>
...
These define the _What, Where, When, and How_ of the backup regime. A storage policy manages subclient data based on business requirements, even when the data resides on multiple different servers in the backup environment.
...
Selecting all six of the BRMS equivalent Control Group check boxes in the Commvault Pre-Defined subclient interface would backup the same
 data set as a Go Save Option 21 – _with the exception that it would not be a bootable image_.
...
| IBM i defined "Parts of the System" | Commvault predefined content | Commvault BMR Minimal OS Tape | Commvault user-defined content | Commvault & BRMS GO Save \ Option 21 | BRMS Option 22 | BRMS Option 23 |
|:-----------------------------------:|:----------------------------:|:-----------------------------:|:------------------------------:|:------------------------------------:|:--------------:|:--------------:|
| Licensed Internal Code              |                              |             yes               |               ×                |                  ×                   |       ×        |                |
| OS/400 Objects in QS YS             |                              |             yes               |               ×                |                  ×                   |       ×        |                |
| User Profiles                       | Security Objects             |             yes               |               ×                |                  ×                   |       ×        |       ×        |
| Private Authorities                 | Security Objects             |             yes               |               ×                |                  ×                   |       ×        |       ×        |
| Configuration Objects               | Configuration Objects        |             yes               |               ×                |                  ×                   |       ×        |       ×        |
| IBM-Supplied Directories            | *Link                        |             yes               |               ×                |                  ×                   |       ×        |                |
| OS/400 Optional Libraries           | *IBM                         |             yes               |               ×                |                  ×                   |       ×        |                |
| Licensed Program Libraries          | *IBM                         |             yes               |               ×                |                  ×                   |       ×        |                |
| IBM Libraries with User Data        | *AllUser                     |             (1)               |               ×                |                  ×                   |                |       ×        |
| User Libraries                      | *AllUser                     |             (1)               |               ×                |                  ×                   |                |       ×        |
| Documents and Folders               | *ALLDLO                      |                               |               ×                |                  ×                   |                |       ×        |
| Distribution Objects                | *ALLDLO                      |                               |               ×                |                  ×                   |                |       ×        |
| User Objects in Directories         | *Link                        |                               |               ×                |                  ×                   |                |       ×        |

 _(1) Only the Commvault Libraries (CVLIB) & CVLIBOBJ) and user profile (CVADMIN, CVBKP), are included_

_**Table 1 – Commvault Pre-Defined Content and Minimal OS Tape image vs. BRMS Options**_
...
1. _Pre-scan script executes_
-- Scan process runs --
1. _Post-scan script executes_
1. _Pre-backup script executes_
1. _(SYNCLIB) Synchronize the libraries to reach check-point)_
1. _(Command to run after Synchronization)_
-- Backup process runs --
1. _Post-backup script runs._

_Note: Any failure in this chain of events can cause the backup to restart or fail as determined by the user and if desired, an email or SNMP alert can be generated._
...
_**Table 2 — Log file names on the IBM i**_
...
1. Exclude files and directories from the restore
1. Add a suffix to each file that you restore (map file)
1. Restore the private authority with the object
1. Restore spool files
1. Allow object differences (has sub-options)
1. Force Object Conversion to current operating system version
1. Job Log output (list, print)

<a name="systemrestore"></a>
...
_Note: None of the system level backups described here include the ibase stage._
